...
public class DemoController {
    ...
    public void getBeanByColumnPrefix(@PathVariable String columnPrefix) throws Exception {
        List<Map> apples = demoService.scanColPre(new Demo(), columnPrefix);
    ...
    public void queryScanAndColumn(@PathVariable String columnPrefix) throws Exception {
        ...
        map.put("avg", "123");
        map.put("content", "123");
        List<Map> apples = demoService.scanValueAndColPre(new Demo(), map, columnPrefix);
        for (Map result : apples) {
            showMap(result);
        }
    }
    ...
    public void firstKeyOnlyFilter() throws Exception {
        ...
        stopWatch.start();
        List<Map> apples = demoService.firstKeyOnlyFilter(new Demo());
        Integer count = 0;
        for (Map result : apples) {
            showMap(result);
            count++;
